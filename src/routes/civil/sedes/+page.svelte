...
<div class="space-y-8">
	...
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
		<div>
			<h2 class="text-2xl font-bold text-brand-text tracking-tight">Análisis de Sedes Judiciales - Civil y Comercial</h2>
							...
							{#each rankResolucion.top as item, i}
								<div class="flex items-center justify-between text-xs py-1 px-2 rounded-lg bg-white border border-brand-border">
									<span class="truncate max-w-[130px] text-brand-text-muted">
										<a href="{base}/civil/sede?id={encodeURIComponent(item.id)}" class="hover:text-brand-indigo transition font-medium font-sans">
											{i + 1}. {item.name}
										</a>
									</span>
